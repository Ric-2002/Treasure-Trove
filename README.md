...
如果本机默认 Node.js 低于 22，使用本机 Node 22 启动 Wrangler：

```bash
PATH="$HOME/.nvm/versions/node/v22.20.0/bin:$PATH" npm run deploy
```
...
- `HEAD /api/health`: 给 `curl -I` 或监控探活使用，只返回状态和响应头。
...
- 历史数据没有 `note` 字段也可以直接解锁使用；页面会在本地补为空备注，下次保存时再写入新结构。
