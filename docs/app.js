(() => {
  ...
  function bindEvents() {
    ...
    elements.entryNote.addEventListener("input", markEditorDraftDirty);
    elements.addCredentialButton.addEventListener("click", () => {
      const row = appendCredentialRow({
  ...
  function readCredentialRows() {
    return Array.from(elements.credentialsList.querySelectorAll(".credential-row"))
      .map((row) => ({
        ...
        note: row.querySelector(".credential-note").value.trim(),
      }))
      .filter((credential) => credential.account || credential.secret || credential.note);
  }
  ...
  function entryMatchesSearch(entry) {
    ...
    const haystack = [
      ...
      ...entry.credentials.map((credential) => credential.note),
    ].join(" ").toLowerCase();
  ...
  function renderCredentialRows(credentials) {
    ...
    const rows =
      credentials.length > 0
        ? credentials
        : [{ id: crypto.randomUUID(), account: "", secret: "", note: "" }];
  ...
  function appendCredentialRow(credential) {
    ...
    row.innerHTML = `
      ...
      <label class="field credential-note-field">
        <span>账户备注</span>
        <textarea class="credential-note" autocomplete="off"></textarea>
      </label>
    `;
    row.querySelector(".credential-account").value = credential.account ?? "";
    row.querySelector(".credential-secret").value = credential.secret ?? "";
    row.querySelector(".credential-note").value = credential.note ?? "";
